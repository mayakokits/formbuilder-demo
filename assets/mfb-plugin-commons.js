(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    module.exports = factory(require('campaignion_vue'));
  } else {
    root.moFormBuilder = root.moFormBuilder || {};
    root.moFormBuilder.pluginCommons = factory(root.campaignion_vue);
  }
}(typeof self !== 'undefined' ? self : this, function (campaignion_vue) {
var mfbPluginCommons =
...
var _collapseTransition = __webpack_require__(2);

exports.default = {
  components: {
    CollapseTransition: _collapseTransition.CollapseTransition
  },
  props: {
    title: String,
    switchable: {
...
module.exports = campaignion_vue.element;
...
var _formKey = __webpack_require__(4);

var _formKey2 = _interopRequireDefault(_formKey);
...
var _LabeledSwitch = __webpack_require__(1);
...
exports.default = {
  ...
  CustomOption: _CustomOption2.default,
  LabeledSwitch: _LabeledSwitch2.default,
  persistPreviewData: _persistPreviewData2.default
};
module.exports = exports['default'];
...
function traverse(current, prefix) {
  ...
  if (current.formKey && typeof current.formKey === 'string') {
    match = current.formKey.match(regex);
    if (match) {
      existingKeys.push(parseInt(match[1], 10));
    }
  }

  for (var i = 0, len = children.length; i < len; i++) {
    traverse(children[i], prefix);
  }
}

function newFormKey(tree, prefix) {
  var i, len;

  existingKeys = [];
  regex = new RegExp('^' + prefix + '_(\\d+)');
  traverse(tree, prefix);
  existingKeys.sort(function (a, b) {
    return a - b;
  });
  for (i = 0, len = existingKeys.length; i < len; i++) {
    if (i !== existingKeys[i]) {
      return prefix + '_' + i;
    }
  }
...
/* This file is written in ES5 - webpack puts this function into the plugins’ UMD loaders. */
  ...
  methods: {
    text: function text(_text) {
      switch (_text) {
        ...
        case 'form key':
          return Drupal.t('Form key');
        case 'edit form key':
          return Drupal.t('Edit');
        case 'description (help text)':
          return Drupal.t('Description (help text)');
      }
    }
  },
...
exports.default = {
  ...
  methods: {
    deletable: function deletable() {
      return this.$store.state.config.originalNode.isRemovable();
    },
    deleteElement: function deleteElement() {
      this.confirmDeletionVisible = false;
      if (this.$store.state.config.originalNode.removeNode()) {
    ...
    text: function text(_text) {
      switch (_text) {
        case 'advanced options':
          return Drupal.t('Advanced options');
        case 'examples for advanced options':
          return Drupal.t('(remove field, wrapper classes, etc.)');
        case 'label position':
          return Drupal.t('Label display');
        case 'private field':
          return Drupal.t('Private');
        case 'wrapper CSS classes':
          return Drupal.t('Wrapper CSS classes');
        case 'remove field':
          return Drupal.t('Remove this field');
        case 'are you sure to delete this?':
          return Drupal.t('Are you sure?');
        case 'cancel deletion':
          return Drupal.t('Cancel');
        case 'confirm deletion':
          return Drupal.t('Confirm');
      }
    }
  },
  template: '<config-section :collapsable="true">\n      <span slot="title">\n        {{text(\'advanced options\')}} <span style="font-weight:normal;">{{text(\'examples for advanced options\')}}</span>\n      </span>\n\n      <el-form-item v-if="typeof element.labelDisplay !== \'undefined\'" :label="text(\'label position\')">\n        <el-select v-model="element.labelDisplay.value">\n          <el-option\n            v-for="item in element.labelDisplay.options"\n            :key="item.value"\n            :label="item.label"\n            :value="item.value">\n          </el-option>\n        </el-select>\n      </el-form-item>\n\n      <labeled-switch v-if="typeof element.private !== \'undefined\'" v-model="element.private" :label="text(\'private field\')" />\n\n      <el-form-item v-if="typeof element.wrapperCls !== \'undefined\'" :label="text(\'wrapper CSS classes\')">\n        <el-input v-model="element.wrapperCls" />\n      </el-form-item>\n\n      <slot></slot>\n\n      <el-form-item>\n        <el-popover v-model="confirmDeletionVisible" popper-class="mfb-delete-element-popover" placement="top" width="230">\n          <el-button slot="reference" type="text" :disabled="!deletable()">{{text(\'remove field\')}}</el-button>\n\n          <p>{{text(\'are you sure to delete this?\')}}</p>\n          <div class="mfb-delete-element-popover-buttons">\n            <el-button type="text" size="mini" @click="confirmDeletionVisible = false">{{text(\'cancel deletion\')}}</el-button>\n            <el-button type="default" size="mini" @click="deleteElement">{{text(\'confirm deletion\')}}</el-button>\n          </div>\n        </el-popover>\n      </el-form-item>\n    </config-section>'
};
module.exports = exports['default'];
...
var DELETE_BUTTON_BREAKPOINT = 470;
...
exports.default = {
  ...
  computed: _extends({}, (0, _vuex.mapState)(['windowWidth'])),
  data: function data() {
    // If not present, add a `default` property to each option.
    ...
    return {
      opts: opts,
      faArrowsAlt: _faArrowsAlt2.default,
      DELETE_BUTTON_BREAKPOINT: DELETE_BUTTON_BREAKPOINT
    };
  },
  ...
  watch: {
    ...
    multiple: function multiple(val) {
      if (!val) {
        // If multiple is turned off, switch off all but the first default options.
        var hitFirstDefault = false;
        this.opts.forEach(function (option) {
          if (option.default && hitFirstDefault) {
            option.default = false;
          } else if (option.default) {
            hitFirstDefault = true;
          }
        });
      }
    }
  ...
  methods: {
    setDefault: function setDefault(index, val) {
      if (val && !this.multiple) {
        // Switch all others off.
        this.opts.map(function (option) {
          option.default = false;
        });
      }
    },
    addOption: function addOption() {
      this.opts.push({ value: '', label: '', default: false });
    },
    ...
    dragEnd: function dragEnd() {
      ...
      document.body.classList.add('dragging-ended');
      setTimeout(function () {
        document.body.classList.remove('dragging-ended');
      }, 50);
    },
    text: function text(_text) {
      switch (_text) {
        ...
        case 'option label':
          return Drupal.t('Label');
        case 'remove option':
          return Drupal.t('Remove');
        case 'add option':
          return Drupal.t('Add Option');
      }
    }
  },
...
exports.default = {
  ...
  methods: {
    text: function text(_text) {
      switch (_text) {
        case 'allow custom option':
          return Drupal.t('Allow "Other..." option');
        case 'custom option text':
          return Drupal.t('Text for "Other..." option');
      }
    }
  },
  template: '\n    <div class="mfb-custom-option">\n\n      <labeled-switch v-model="on" @change="$emit(\'update:enabled\', $event)" :label="text(\'allow custom option\')" />\n\n      <collapse-transition>\n        <el-form-item v-if="on" :label="text(\'custom option text\')">\n          <el-input v-model="str" @input="$emit(\'update:label\', $event)" size="small" />\n        </el-form-item>\n      </collapse-transition>\n    </div>\n  '
};
module.exports = exports['default'];
...
// This mixin enables preview components to perseist their $data while they are
// being dragged.

exports.default = {
  created: function created() {
    var persistedData = this.element.getPreviewData();
    if (persistedData) {
      for (var key in persistedData) {
        if (persistedData.hasOwnProperty(key)) {
          _vue2.default.set(this.$data, key, persistedData[key]);
        }
      }
    }
  },
  ...
  watch: {
    $data: {
      handler: function handler(val) {
        this.element.setPreviewData(val);
      },
...
/***/ })
/******/ ]);
return mfbPluginCommons;
}));
//# sourceMappingURL=mfb-plugin-commons.js.map